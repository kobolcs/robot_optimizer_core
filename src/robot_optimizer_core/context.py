...
def analyze_with_context(project_path: str, config: ApplicationConfig | None = None):
    """Example of using application context instead of globals."""
    # Create application context (not global!)
    with create_application(config) as app:
        ...
        with app.metrics.timer("analysis.total"):
            ...
            for file_path in files:
                ...
                with app.request_scope(file=str(file_path)) as scope:
...
# Example: Testing without global state
def test_analyzer_without_globals():
...
# Example: Multiple isolated contexts
