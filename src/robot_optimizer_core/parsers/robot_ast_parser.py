...
from robot.parsing.model import (
    Statement, KeywordSection, TestCaseSection, VariableSection,
    Keyword, TestCase, Variable
)
from robot.parsing.suitestructure import SuiteStructureVisitor

from ..domain.entities import TestFile
from ..domain.value_objects.robot_ast import (
    RobotSuite, RobotKeyword, RobotTestCase, RobotVariable,
    RobotImport, RobotArgument, KeywordCall
)
...
class RobotASTParser(RobotParserRepository):
    ...
    def parse_suite(self, test_file: TestFile) -> RobotSuite:
        ...
        try:
            ...
            return RobotSuite(
                ...
                source=test_file.path,
                documentation=f"Parse error: {str(e)}",
                metadata={},
                imports=[],
                variables=[],
                keywords=[],
    ...
    def _extract_keyword_calls(self, body: List[Any], file_path: Path, 
                              ...
                              parent_keyword: Optional[str] = None) -> List[KeywordCall]:
    ...
    def _parse_keyword_call(self, item: Any, file_path: Path,
                           parent_test: Optional[str] = None,
                           parent_keyword: Optional[str] = None) -> Optional[KeywordCall]:
    ...
    def _get_suite_documentation(self, model: Any) -> Optional[str]:
